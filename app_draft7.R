...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    sidebarLayout(
      ...
      mainPanel(
        h4("Distribution Table"),
        dataTableOutput("distTable"),
        h4("Distribution Bar Chart"),
        plotOutput("distPlot")
      )
    )
  ),
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput(inputId = "HPM1",label = "Housing Project Metric 1:",
                    choices = names(select_if(startData,is.numeric)),
                    selected = "TOTAL_DEVELOPMENT_COST"),
  ...
  fluidRow(
    ...
    sidebarLayout(
      ...
      # Show a plot of the generated distribution
      mainPanel(
        h4("Table: Time Series by Date of Funding"),
        dataTableOutput("distDateTable"),
...
server <- function(input, output) {
  ...
  output$distDateTable <- renderDT(
    dateTimeData(), rownames = F, extensions = 'Buttons', filter = "top", 
    editable = TRUE,
    options = list(
      dom = 'Blfrtip',
      buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
