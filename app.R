...
ui <- fluidPage(
  ...
  h3("Box Plots of Housing Project Characteristics"),
...
#  })
  ...
  output$boxPlot <- renderPlot({
    startData %>% 
      ggplot(aes(x=!!as.name(input$GBBox),y=!!as.name(input$MetricBox),fill=!!as.name(input$GBBox))) +
      geom_boxplot() +
      coord_flip()  +
      xlab("") +
  ...
  output$groupedLinePlot <- renderPlot({
    startData %>% 
      ...
      ggtitle(paste("TOTAL ", str_replace_all(input$HPM, "_", " "), " FOR ", 
                    str_replace_all(input$housingType, "_", " "),
